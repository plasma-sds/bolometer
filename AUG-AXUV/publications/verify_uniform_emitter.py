...
from axuv.io import PROJECT_ROOT, load_axuv_df
...
def _verify_sector(sector, axuv_df, uniform_mat):
    ...
    world, cameras = create_observable_world(
        ...
        axuv_df=axuv_df,
        cad_mesh=False,
        etendue_mode=False,  # adds absorbing rectangular vessel wall
    )
...
def _plot_and_save(results, sector):
    ...
    for ext in ("png", "eps"):
        fig.savefig(
            OUTPUT_DIR / f"verify_uniform_{sector}_parity.{ext}",
    ...
    for ext in ("png", "eps"):
        fig.savefig(
            OUTPUT_DIR / f"verify_uniform_{sector}_rel_err.{ext}",
            format=ext if ext == "eps" else None,
            bbox_inches="tight",
        )
    plt.close(fig)
