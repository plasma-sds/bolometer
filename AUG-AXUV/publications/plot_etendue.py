...
df = pd.read_csv(AXUV_DATAFILE, sep=r"\s+", engine="python").drop(
    columns=[
        "chan", "act", "con", "R_Kabel", "U_Gen.", "f_Blende", "f_Folie", "d(Folie-Blende)", "delta", "gamma", "alpha", "R_start", "z_start", "Phi_start", "R_end", "z_end", "Phi_end", "F", "Foil_ID"
    ]
)
...
print(f"First 2 raytraced: {list(zip(raytraced['names'][:2], raytraced['etendue'][:2]))}")
...
for x, label in zip(quarter_centers, quarter_labels):
    ax.text(
        x, 0.02,              # x: quarter center, y: near bottom
        label,
